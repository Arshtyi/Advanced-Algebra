...
\thm{相似关系的扩张}{相似关系的扩张}{
    设$\bm{A},\bm{B}\in M_n\left(\bbf \right),\bbf \subseteq \mathbb{K}$，则$\bm{A},\bm{B}$在$\bbf $上相似当且仅当$\bm{A},\bm{B}$在$\mathbb{K}$上相似.\begin{proof}
        因为$\bm{A},\bm{B}$在$\bbf $相似即存在$\bm{P}\in M_n\left(\bbf \right)\st\bm{B}=\bm{P}^{-1}\bm{AP}$.另一方面，$\bm{A},\bm{B}$在$\mathbb{K}$相似即存在$\bm{Q}\in M_n\left(\mathbb{K}\right)\st\bm{B}=\bm{Q}^{-1}\bm{AQ}$.
