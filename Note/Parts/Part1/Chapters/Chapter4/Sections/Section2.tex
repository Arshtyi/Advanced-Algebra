...
\subsection{运算}
\dfn{线性映射全体}{线性映射全体}{
    设$V_{\bbk }$、$U_{\bbk }$,记$\call   \left(V,U\right)$为$V \longrightarrow U$的线性映射全体.
}
...
\thm{}{线性映射全体是线性空间}{
    $\call   \left(V,U\right)$在\cref{def:线性映射的加法与数乘}的加法和数乘运算下是$\bbk $上的线性空间.
...
\subsection{代数}
\dfn{代数}{代数}{
    设$\bbk $上的线性空间$A$,如果定义一种乘法运算
    \begin{align*}
        \cdot :
        A \times A       & \longrightarrow A    \\
        \left(a,b\right) & \longmapsto a\cdot b
    \end{align*}
    若$\forall a,b,c\in A,\forall k \in \bbk $,满足\begin{enumerate}[label=\arabic*)]
        \item 结合律:$
        ...
        \item 与数乘的相容性:
              ...
              $
    \end{enumerate}
    那么称$A$为$\bbk $上的代数即$\bm{A}$是一个$\bbk $-代数,单位元$e$称为$A$的恒等元.
}
\exa{}{}{
...
\exa{}{}{
    若$\bbk _1 \subseteq\bbk _2$,定义$\bbk _2$中的乘法是数的乘法,则$\bbk _2$是$\bbk _1$-代数.
}
\exa{}{}{
    设线性空间$M_n\left(
        \bbk
        \right)$,定义乘法为矩阵的乘法,
    则$M_n\left(\bbk \right)$
    是$\bbk $-代数.
}
\exa{}{}{
    \cref{def:线性映射全体}提到$\call   \left(V_{\bbk }
        \right)$是定义有乘法的,那么它是$\bbk $-代数.
}
\subsection{幂}
\dfn{线性映射的幂}{线性映射的幂}{
...
\cor{}{自同构的保持}{
    ...
    特别地,$\forall k\neq 0 \in \bbk $,则$k\cdot \bm{\varphi}$为自同构且
