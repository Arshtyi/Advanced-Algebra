...
\rem{}{}{
    在\cref{def:线性同构}定义线性空间之间的同构的时候，我们先要求双射，再要求保持线性组合，\cref{def:线性映射}反过来先要求线性组合再要求双射.两种定义是相同的.
}
\exa{}{}{
    \[
        \bm{\varphi}:
...
\exa{}{}{
    设$\mathbb{K}$上的线性空间$V$，取定$k \in \mathbb{K}$，则\begin{align*}
        \bm{\varphi}:
        V           & \longrightarrow V \\
        \bm{\alpha} & \longmapsto
        k\bm{\alpha}
    ...
    \end{enumerate}\begin{proof}
        \begin{enumerate}[label=\arabic*）]
            ...
            \item 因为$\bm{\varphi}$为双射，则$\bm{\varphi}^{-1}$为双射，只要证
                  $\bm{\varphi}^{-1}$保持线性组合即可.任取$\bm{x},\bm{y}\in U,k,l\in\mathbb{K}$，只需证
                  \[
