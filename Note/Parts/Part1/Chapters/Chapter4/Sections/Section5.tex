...
\subsection{定义}
\dfn{不变子空间}{不变子空间}{
    设线性映射$\bm{\varphi}\in \mathcal{L}\left(V_{\mathbb{K}}\right)$，$U$是$V$的子空间.若
    \[
...
\exa{}{}{
    考虑$V=\mathbb{R}^2$，线性变换$\bm{\varphi}$为绕原点逆时针旋转$\theta$角，其在标准基下的表示矩阵为
    \[
        \bm{A}=\begin{bmatrix}
            \cos \theta & -\sin \theta \\
            \sin \theta & \cos \theta
...
\thm{}{不变子空间诱导的线性变换的表示矩阵}{
    设线性变换$\bm{\varphi}\in \mathcal{L}\left(V^n\right)$，且$U$为$\bm{\varphi}$-不变子空间，取$U$的一组基
...
\cor{}{全空间分解为两个不变子空间的直和}{
    设线性变换$\bm{\varphi}\in \mathcal{L}\left(V^n\right)$，且$V=V_1\oplus V_2$（$V_1$、$V_2$均为$\bm{\varphi}$-不变子空间），那么一定可以分别取$V_1$、$V_2$的一组基来拼成$V$的一组基，使得
...
设线性变换$\bm{\varphi}\in \mathcal{L}\left(V^n
...
\subsection{例子}
\exa{}{}{
    设$V^3$的一组基为$\left\{\bm{e}_1,\bm{e}_2,\bm{e}_3\right\}$，线性变换$\bm{\varphi}\in \mathcal{L}\left(V\right)$在这组基下的表示矩阵为
    \[\bm{A}=
        \begin{pmatrix}
            3 & 1 & -1 \\
            2 & 2 & -1 \\
...
\subsection{代数与几何}
\exa{}{}{
    设线性映射$\bm{\varphi}\in \mathcal{L}\left(V^n,U^m\right)$，$\rmr \left(\bm{\varphi}\right)=r\geqslant 1$，证明：
...
\exa{}{}{
    设$\bm{\varphi}\in \mathcal{L}\left(V^n\right)$，且$\bm{\varphi}^m=\bm{0}$,$n=mq+1$，证明：
...
\exa{像空间与核空间稳定}{像空间与核空间稳定}{
    ...
    \[
        ...
        \right)=\cdots
    \]\begin{proof}
        我们从几何角度给出一个更强的结论：设$\bm{\varphi}\in \mathcal{L}
...
\exa{}{}{
    设$\bm{A}$、$\bm{B}\in M_{m\times n}\left(\mathbb{K}\right)$，证明：$\bm{Ax}=\bm{0}$与$\bm{Bx}=\bm{0}$同解等价于存在$m$阶非异阵$\bm{P}$使得$\bm{B}=\bm{PA}$\begin{proof}
        即证：已知$\bm{\varphi}$、$\bm{\psi}\in \mathcal{L}\left(V^n,U^m\right)$，那么$\Ker \bm{\varphi}=\Ker \bm{\psi}$等价于
