...
\subsection{结式}
\thm{基域扩张}{基域扩张}{
    设$f\left(x\right),g\left(x\right)\in\bbk \left[x\right]$,基于基域扩张下的不变性,有:$\left(
        f\left(x\right),g\left(x\right)
        \right)_{\bbk }=1\Longleftrightarrow
        \left(
        f\left(x\right),g\left(x\right)
...
\lem{}{不互素的条件}{
    设$f\left(x\right),g\left(x\right)\in\bbk \left[x\right],d\left(x\right)=\left(
        f\left(x\right),g\left(x\right)
        \right)$,则$d\left(x\right)\neq 1$当且仅当$\exists u\left(x\right)\neq 0,v\left(x\right)\neq 0\in\bbk \left[x\right]\st$
    \[
...
\dfn{结式/Sylvester行列式}{结式}{
    ...
    定义一个循环映射
    \begin{align*}
        r:\bbk _{n+m}                                 & \longrightarrow\bbk _{n+m}                                     \\
...
\thm{结式的根表示}{结式的根表示}{
    ...
    \]\begin{proof}
        ...
        则一方面,若$\exists 1\leqslant i\leqslant n,1\leqslant j\leqslant m\st x_i=y_j,LHS=0=RHS.$
