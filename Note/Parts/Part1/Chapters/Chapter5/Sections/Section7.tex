...
\thm{实数域上的不可约多项式}{实数域上的不可约多项式}{
    ...
    \]\begin{proof}
        任取$\mathbb{R}$上的不可约多项式，其一，$\deg f\left(x\right)=1$显然成立.
        ...
        其二，$\deg f\left(x\right)=2$时设$f\left(x\right)=ax^2+bx+c\left(a\neq 0\right)$，则$f\left(x\right)$可约当且仅当$f\left(x\right)=a\left(x-x_1\right)\left(x-x_2\right)
            \left(x_1,x_2\in\mathbb{R}\right)$.于是$f\left(x\right)$不可约当且仅当其无实根即判别式小于零.
        ...
        \]
        其中$q\left(x\right),r\left(x\right)\in\mathbb{R}\left[x\right],\deg r\left(x\right)<\deg \left(\left(x-a\right)^2+b^2\right)=2$.代入$x=a+b\mathrm{i}$得$0=r\left(a+b\mathrm{i}\right)$，结合次数即得
        $r\left(x\right)=0.$
    \end{proof}
}
