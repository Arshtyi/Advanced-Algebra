...
\subsection{定义}
\dfn{单项式}{单项式}{
设$\bbk $为数域,$x_1,x_2,\cdots,x_n$是未定元,则
\[
    ax_1^{i_1}x_2^{i_2}\cdots x_n^{i_n}
\]
(其中$a\in\bbk $称为系数,$i_1,i_2,\cdots,i_n\in\bbn $)称为一个单项式.
...
\dfn{单项式的运算}{单项式的运算}{
同类项的加法即是合并同类项,非同类项的加法即是形式和；$\forall k \in\bbk $,数乘定义为
...
\dfn{多元多项式全体}{多元多项式全体}{
记$\bbk $上的$n$元多形式全体为\[\bbk \left[x_1,x_2,\cdots,x_n\right]=\left\{
    ...
    \right\}\]
这是一个$\bbk $-代数,称为关于$x_1,x_2,\cdots,x_n$的$n$元多项式代数(环).
}
\rem{}{}{
    一个自然的问题是,$\bbk \left[x\right]$的各种性质是否可以推广到$\bbk \left[x_1,x_2,\cdots,x_n\right]$.
}
...
\cor{多元多项式环的乘法消去律}{}{
    由\cref{thm:多元多项式乘法的整性}整性立刻得到,多元多项式环满足乘法消去律:设 $f,g,h\in\bbk \left[x_1,x_2,\cdots,x_n\right]$,且 $f\neq 0$,则
...
\thm{齐次分解}{齐次分解}{
    \begin{enumerate}[label=\arabic*)]
        ...
        \item 设$f\in\bbk \left[x_1,x_2,\cdots,x_n\right]$,则一定可以通过限制定义域得到与\[
...
\lem{}{多元多项式运算结果的次数}{
    设$f$、$g\in\bbk \left[x_1,x_2,\cdots,x_n\right]$,则\begin{enumerate}[label=\arabic*)]
...
\lem{非零多项式一定是非零函数}{非零多项式一定是非零函数}{
    设$0\neq f\in\bbk \left[
            x_1,x_2,\cdots,x_n
            \right]$,则存在$a_1,a_2,\cdots,a_n\in\bbk \st$
    \[  f\left(
        a_1,a_2,\cdots,a_n
        \right)\neq 0 \]\begin{proof}
        ...
        其中$b_m\left(
            x_1,x_2,\cdots,x_{n-1}
            \right)\neq 0$,那么由归纳假设,$\exists a_1,a_2,\cdots,a_{n-1}\in\bbk \st$
        \[
            b_m\left(
...
\cor{多元多项式相等等价于多元函数相等}{多元多项式相等等价于多元函数相等}{
    设$f$、$g\in\bbk \left[
        ...
        设$f$、$g\in\bbk \left[
                x_1,x_2,\cdots,x_n
                \right]$且$\overline{f}=\overline{g},f\neq g$则$f-g\neq 0$,由\cref{lem:非零多项式一定是非零函数}$\exists a_1,a_2,\cdots,a_n\in\bbk \st$
        \[
            \left(f-g\right)
            \left(
            a_1,a_2,\cdots,a_n
...
\exa{}{}{
    考虑$f=x,g=y\in\bbk \left[x,y\right]$,容易有$\left(f,g\right)=1$且$\forall
        u\left(x,y\right),v\left(x,y\right)\in\bbk \left[x,y\right]$
    \[
...
\thm{多元多项式的因式分解}{多元多项式的因式分解}{
    ...
    $
        ...
        \right)$是$\bbk $上的 $n$ 元不可约多项式.并且这一分解在相伴定义下是唯一的.\begin{proof}
        该证明超过高等代数范畴.
    \end{proof}
}
\clm{}{}{
    基于\cref{thm:多元多项式的因式分解},我们将$n$ 元多项式环$\bbk \left[
            x_1,x_2,\cdots,x_n
            \right]$称为唯一分解整区(UFD).
}