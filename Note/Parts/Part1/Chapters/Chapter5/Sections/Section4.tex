...
\dfn{可约多项式与不可约多项式}{可约多项式与不可约多项式}{
    设$f\left(x\right)\in\bbk \left[x\right]$,
    $\deg f\left(x\right)\geqslant 1$.若存在$g\left(x\right),h\left(x\right)\in\bbk \left[x\right]$,且
    $\deg g\left(x\right)<\deg f\left(x\right),\deg h\left(x\right)<\deg f\left(x\right)$\st
    \[f\left(x\right)=g\left(x\right)h\left(x\right)\]
    那么称$f\left(x\right)$在$\bbk \left[x\right]$上是可约的.否则称为是不可约的.
}
\clm{一次多项式的不可约性}{}{
    多项式的可约与否取决于数域的选择.但不管在哪个域上,一次多项式总是不可约的.
}
\dfn{可约的等价定义}{可约的等价定义}{
    设$f\left(x\right)\in\bbk \left[x\right]$,$\deg f\left(x\right)\geqslant 1$.若存在多项式$g\left(x\right),h\left(x\right)\in\bbk \left[x\right]$,且
    $\deg g\left(x\right)>0,\deg h\left(x\right)>0$\st
    \[f\left(x\right)=g\left(x\right)h\left(x\right)\]那么称$f\left(x\right)$在$\bbk \left[x\right]$上是可约的.
}
\dfn{不可约的等价定义}{不可约的等价定义}{
    设$f\left(x\right)\in\bbk \left[x\right],\deg f\left(x\right)\geqslant 1$,则$f\left(x\right)$不可约等价于$f\left(x\right)$的因式有且仅有$c\neq 0$和$cf\left(x\right)$.
}
\exa{}{}{
    $f\left(x\right)=x^2-2$在$\bbq  \left[x\right]$不可约,在$\bbr \left[x\right]$可约.
}
\lem{}{不可约多项式与其他多项式}{
    设$p\left(x\right)$为$\bbk \left[x\right]$
    上的一个不可约多项式,$f\left(x\right)\in \bbk \left[x\right]$,则要么$\left(p\left(x\right),f\left(x\right)\right)=1$,要么
...
\thm{}{不可约多项式作为因子不会是两个多项式的因子乘法得到的}{
    设不可约多项式$p\left(x\right)\in\bbk \left[x\right]$,
    且有多项式$f\left(x\right),g\left(x\right)\in\bbk \left[x\right]$且$p\left(x\right)\mid f\left(x\right)g\left(x\right)$,则
...
\subsection{因式分解}
\thm{因式分解定理}{因式分解定理}{
    设$f\left(x\right)\in\bbk \left[x\right]$且$\deg f\left(x\right)\leqslant 1$,则\begin{enumerate}[label=\arabic*)]
        \item 存在性:$f\left(x\right)$一定可分解为$\bbk $上有限多个不可约多项式之积
        \item 唯一性:设\[
              ...
              \]
              是$f\left(x\right)$的两个不可约分解,即$p_i\left(x\right),q_j\left(x\right)$均为$\bbk $上的次数大于零的不可约多项式,则一定有$s=t$且在适当调换因式的次序后,有\[
    ...
    \end{enumerate}\begin{proof}
        \begin{enumerate}[label=\arabic*)]
            \item 对$\deg f\left(x\right)=n$归纳,$n=1$显然成立.
                  ...
                  \[f\left(x\right)=g\left(x\right)h\left(x\right)\]其中$g\left(x\right),h\left(x\right)\in\bbk \left[x\right],\deg g\left(x\right)<n,\deg h\left(x\right)<n$.
                  则由归纳假设有
                  \[g\left(x\right)=p_1\left(x\right)\cdots p_s\left(x\right),h\left(x\right)=q_1\left(x\right)q_2\left(x\right)\cdots q_t\left(x\right)\]其中$p_i\left(x\right),q_j\left(x\right)$均为$\bbk \left[x\right]$上的不可约多项式.于是存在性证毕.
...
\thm{标准因式分解}{标准因式分解}{
    设$f\left(x\right)\in\bbk \left[x\right],\deg f\left(x\right)\geqslant 1$,则有如下标准因式分解
    \[
        f\left(x\right)=cp_1\left(x\right)^{e_1}p_2\left(x\right)^{e_2}\cdots p_m\left(x\right)^{e_m}
    \]
    其中$c\neq 0\in\bbk $,$p_i\left(x\right)\left(1\leqslant i\leqslant m\right)$为互异的首一不可约多项式,$ e_i\geqslant 1\left(\forall1\leqslant i\leqslant m\right)$.
}
\dfn{重因式}{重因式}{
    若标准因式分解中$e_1>1\left(\forall 1\leqslant i\leqslant m\right)$,则$p_i\left(x\right)$称为$f\left(x\right)$的$e_i$重因式.$e_i=1$时称为单因式.
}
\cor{公共因式分解}{公共因式分解}{
    设$f\left(x\right),g\left(x\right)\in\bbk \left[x\right]$,则添加某些不可约多项式的零次幂后,有如下公共因式分解
    \begin{align*}
    ...
    \end{align*}
    其中$c_1\neq 0,c_2\neq 0\in\bbk $,$p_i\left(x\right)\left(1\leqslant i\leqslant n\right)$为互异的首一不可约多项式,$e_i\geqslant 0,f_i\geqslant 0\left(\forall 1\leqslant i\leqslant n\right)$.
}
...
\dfn{形式导数}{形式导数}{
设$f\left(x\right)=a_nx^n+a_{n-1}x^{n-1}+\cdots+a_1x+a_0\in\bbk \left[x\right]$,则
\[
...
\cor{}{相同的不可约因子}{
    设多项式$f\left(x\right)\in\bbk \left[x\right],\deg f\left(x\right)\geqslant 1$,其形式导数为$f'\left(x\right)$,$d\left(x\right)=\left(f\left(x\right),f'\left(x\right)\right)$,则$\displaystyle
        \frac{f\left(x\right)}{d\left(x\right)}$一定没有重因式且与$f\left(x\right)$有相同的不可约因子(不计重数).\begin{proof}
        设标准分解
        \[
            f\left(x\right)=cp_1\left(x\right)^{e_1}p_2\left(x\right)^{e_2}\cdots p_m\left(x\right)^{e_m}
        \]
        其中$c\neq 0\in\bbk $,$p_i\left(x\right)\left(1\leqslant i\leqslant m\right)$为互异的首一不可约多项式,$e_i\geqslant 1\left(1\leqslant i\leqslant m\right)$.求导即有
        \begin{align*}
...
\thm{重因式判定}{重因式判定}{
    设$f\left(x\right)\in\bbk \left[x\right],\deg f\left(x\right)\geqslant 1$,则$f\left(x\right)$无重因式等价于
