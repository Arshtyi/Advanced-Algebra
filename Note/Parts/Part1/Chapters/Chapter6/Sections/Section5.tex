...
\exa{}{}{
    设矩阵$\bm{A}^m\in M_m\left(
        \bbk
        \right),\bm{B}^n\in M_n\left(\bbk \right)$无公共特征值,则$\forall \bm{C}\in M_{m\times n}\left(\bbk \right)$,矩阵方程$\bm{AX}-\bm{XB}=\bm{C}$有唯一解.\begin{proof}
        考虑$\bm{\varphi}\in \call  \left(
            M_{m\times n}\left(\bbk \right)
            \right):\bm{X}\longmapsto
            \bm{AX}-\bm{XB}$,由上例可知$\bm{\varphi}$是单射,那么也是满射,于是是线性同构.于是$\forall \bm{C}\in M_{m\times n}\left(\bbk \right)$,一定存在唯一一个$\bm{X}_0\in M_{m\times n}\left(\bbk \right)$使得\[\bm{\varphi}\left(\bm{X}_0\right)=\bm{AX}_0-\bm{X}_0\bm{B}=\bm{C}\qedhere\]
    \end{proof}
}
\exa{}{}{
...
\exa{}{}{
    设$\bm{A}$为非异循环矩阵,求证:$\bm{A}^{-1}$也是循环矩阵.\begin{proof}
        ...
        \]并有基础循环矩阵$\bm{J}=\begin{pmatrix}
            ...
            f\left(\lambda\right)=\lambda^n-1\Longrightarrow\omega_k=e^{\frac{2k\pi \rmi }{n}},0\leqslant k\leqslant n-1
