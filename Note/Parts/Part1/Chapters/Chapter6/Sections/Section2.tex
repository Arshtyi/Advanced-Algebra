...
\thm{几何意义的可对角化条件}{几何意义的可对角化条件}{
    设$V$是$\bbk $上的$n$维线性空间,$\bm{\varphi}\in\call  \left(V\right)$.则$\bm{\varphi}$可对角化当且仅当$\bm{\varphi}$有$n$个线性无关的特征向量.\begin{proof}
...
\exa{}{}{
    ...
    \]其特征值为$1,1$,但只有一个线性无关的特征向量$k\begin{pmatrix}
            1 \\0
        \end{pmatrix}\left(
        k\in\bbk \backslash \left\{0\right\}
        \right)$,故不可对角化.
