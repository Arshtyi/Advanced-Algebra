...
\dfn{坐标向量}{坐标向量}{
    固定$V_{\bbk }$的一组基$\left\{\bm{e}_1,\bm{e}_2,\cdots
...
\subsection{同构}
\dfn{线性同构}{线性同构}{
设$V_{\bbk },U_{\bbk }$,若存在一个双射$\varphi    : V \longrightarrow U$使得
...
\thm{}{与标准行(列)向量空间同构}{
    $\bbk $上任一$n$维线性空间$V$均与$n$维行(列)向量空间$\bbk^n\left(\bbk ^n\right)$同构.\begin{proof}
