...
设$V$为酉空间,标准正交基为$\left\{
    \bm{e}_1,\bm{e}_2,\cdots,\bm{e}_n
    \right\}$,线性同构\begin{align*}
    \bm{\varphi}:V & \longrightarrow\bbc ^n\text{标准内积}
...
\dfn{保积同构}{保积同构}{
    设$V,U$是$\bbk $上的实(复)内积空间,线性映射$\bm{\varphi}:V\longrightarrow U$,若$\forall\bm{\alpha},\bm{\beta}\in V:$\[
        \left(
        \bm{\varphi}\left(\bm{\alpha}\right),\bm{\varphi}\left(\bm{\beta}\right)
        \right)_U=\left(
        \bm{\alpha},\bm{\beta}
...
\exa{}{}{
    嵌入映射:\begin{align*}
        \bm{\varphi}:\bbr ^2 & \longrightarrow\bbr ^3\text{标准内积} \\
        \begin{pmatrix}
            a \\b
...
\cor{}{内积空间保积同构的等价条件}{
    设$V,U$同为有限维实(复)内积空间,则$V,U$间存在保积同构$\bm{\varphi}:V\longrightarrow U$当且仅当$\dim V=\dim U$.\begin{proof}
        ...
        \end{align*}存在,扩张为$\bm{\varphi}\in\call \left(V,U\right)$,容易知道将一组基映为另一组基的线性映射一定是线性同构,根据\cref{prop:保持内积的等价结论}即得.\qedhere
    \end{proof}
}
\subsection{正交变换、酉变换}
...
\dfn{正交矩阵、酉矩阵}{正交矩阵、酉矩阵}{
    设$\bm{A}\in M_n\left(\bbr \right)$,若$\bm{A}'=\bm{A}^{-1}$,则称$\bm{A}$是正交矩阵；设$\bm{C}\in M_n\left(\bbc \right)$,若$\overline{\bm{C}}'=\bm{C}^{-1}$,则称$\bm{C}$是酉矩阵.
}
\thm{算子与矩阵}{算子与矩阵}{
...
\thm{正交阵的判定}{正交阵的判定}{
    设矩阵$\bm{A}\in M_n\left(\bbr \right)$,则下列结论等价\begin{enumerate}[label=\arabic*)]
        \item $\bm{A}$为正交阵
        \item $\bm{A}$的$n$个行向量是$\bbr _n$(标准内积)的一组标准正交基
        \item $\bm{A}$的$n$个列向量是$\bbr ^n$(标准内积)的一组标准正交基
    \end{enumerate}\begin{proof}
        因为$\bm{A}$为正交阵即\[
        ...
        \]作$\bm{A}=\begin{pmatrix}
            ...
            \right)
        \end{align*}其中$\left(i,j\right)$元即为$\delta_{ij}=\bm{\alpha}_i\cdot\bm{\alpha}_j'=\left(\bm{\alpha}_i,\bm{\alpha}_j\right)_{\bbr _n}$,于是得证.列形式同理.
    \end{proof}
}
\thm{酉矩阵的判定}{酉矩阵的判定}{
    设矩阵$\bm{C}\in M_n\left(\bbc \right)$,则下列结论等价\begin{enumerate}[label=\arabic*)]
        \item $\bm{C}$为酉矩阵
        \item $\bm{C}$的$n$个行向量是$\bbc _n$(标准内积)的一组标准正交基
        \item $\bm{C}$的$n$个列向量是$\bbc ^n$(标准内积)的一组标准正交基
    \end{enumerate}
}
\clm{}{}{
    正交阵是特殊的酉阵.
...
\pro{}{酉阵的行列式}{
    酉阵的行列式模长为$1$(特别地,正交阵的行列式等于$\pm1$),特征值模长为$1$.\begin{proof}
        ...
        设$\bm{A\alpha}=\lambda_0\bm{\alpha},\bm{0}\neq\bm{\alpha}\in \bbc ^n,\lambda_0\in\bbc $.于是\[
...
\thm{$\bm{QR}$分解}{QR分解}{
    ...
    \]其中$\bm{Q}$为正交阵(酉阵),$\bm{R}$为主对角元全部大于等于$0$的上三角阵.进一步地,若$\bm{A}$非异,则上述分解唯一.\begin{proof}
        作列分块$\bm{A}=\left(\bm{u}_1,\bm{u}_2,\cdots,\bm{u}_n\right)$,做一个推广的Gram-Schmidt正交化,得到正交向量组\[\left\{
            ...
            \right\}\]其中$\bm{w}_i$要么是零向量,要么是单位向量.

        考虑数学归纳法,设$\bm{w}_1,\bm{w}_2,\cdots,\bm{w}_{k-1}$已经做好,考虑$\bm{w}_k.$令\[
