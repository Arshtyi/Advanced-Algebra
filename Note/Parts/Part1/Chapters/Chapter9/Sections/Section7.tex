...
\lem{}{正规与多项式相容}{
    设$V$为$n$维欧式空间，$\bm{\varphi}$是$V$上的正规算子，则$\forall f\left(x\right)\in\mathbb{R}\left[x\right],f\left(\bm{\varphi}\right)$也是$V$上的正规算子.\begin{proof}
...
\thm{}{极小多项式不可约分解引出的正交直和分解}{
    设$n$维欧式空间$V$上的实正规算子$\bm{\varphi}$的极小多项式为$g\left(x\right)$，设$g_1\left(x\right),g_2\left(x\right),\cdots,g_k\left(x\right)$是$g\left(x\right)$所有互不相同的首一不可约多项式，$\forall1\leqslant i\leqslant k:W_i\coloneqq \mathrm{Ker}\,g_i\left(\bm{\varphi}\right)$，则\begin{enumerate}[label=\arabic*）]
